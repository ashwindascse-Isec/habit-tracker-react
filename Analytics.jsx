...
export default function Analytics({ habits, completions }) {
  ...
  // Only re-calculates weekly progression loops when completions map changes
  const weekData = useMemo(() => {
    return Array.from({ length: 7 }, (_, i) => i - 6).map(offset => {
  ...
  // Caches and calculates habit categories dynamically
  const cats = useMemo(() => {
  ...
  // Only re-sorts leaderboard list items when a streak value actually alters
